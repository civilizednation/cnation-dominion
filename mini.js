(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .app.mini-mode .mini-effect.compact { -webkit-line-clamp: 2; font-size: 8px; }
      ...
      @media (min-width: 561px) {
        .app.mini-mode .supply-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); }
        .app.mini-mode .player .hand { grid-template-columns: repeat(7, minmax(0, 1fr)); }
      }
      @media (orientation: landscape) and (max-height: 560px) {
        ...
        .app.mini-mode .supply-card.mini-supply { min-height: 76px; }
      }
    `;
  ...
  window.CNationMini = {
    ...
    handHtml(c, index, human) {
      ...
      return `<button type="button" class="mini-card mini-text-card type-${kind} ${playable ? "" : "disabled"} ${selected ? "selected" : ""}" data-hand="${index}" aria-label="${text(cd.name)}, 비용 ${cd.cost}, ${text(EFFECTS[c.id] || "")}">${cardBody(c.id, true)}</button>`;
    },
    ...
    supplyHtml(id) {
      ...
      return `<button type="button" class="supply-card mini-supply type-${kind} ${count <= 0 ? "empty" : ""}" data-buy="${id}" aria-label="${text(cd.name)}, 비용 ${cd.cost}, 남은 카드 ${count}, ${text(EFFECTS[id] || "")}">${cardBody(id)}<span class="mini-count" aria-label="남은 수량 ${count}">${count}</span></button>`;
    },

    modalCardHtml(item) {
      const cd = cards[item.id];
      const kind = typeKey(cd);
      return `<button type="button" class="supply-card mini-supply type-${kind}" data-pick="${text(item.uid)}" aria-label="${text(cd.name)}, 비용 ${cd.cost}, ${text(EFFECTS[item.id] || "")}">${cardBody(item.id)}</button>`;
    }
  };
})();
