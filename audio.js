(() => {
  ...
  // URL of the kingdom's first track — the only one a loading screen needs to wait on. The
  // second track is preloaded later, well before it's needed, via prepareNextTrack() during
  // actual playback.
  function kingdomFirstTrackUrl(presetIndex = 0) {
    return bgmUrl((KINGDOM_BGM[presetIndex] || KINGDOM_BGM[0])[0]);
  }
