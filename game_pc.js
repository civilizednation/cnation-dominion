...
const presets = [
  ...
  ["무한의 엔진","액션과 드로우를 연쇄하여 한 턴에 수많은 카드를 사용하는 덱","지하옥, 마을, 관료, 나무꾼, 작업장, 개조, 대장장이, 축제, 실험실, 시장"],
  ...
  ["비밀 거래","카드 획득과 변환을 통해 순식간에 고성능 덱을 완성하는 덱","작업장, 개조, 연회, 대부업자, 민병대, 마을, 대장장이, 광산, 의회의사당, 시장"],
...
function confirmThiefGain(revealed, trashed) {
  return new Promise(resolve => {
    ...
    $("modalCards").innerHTML = revealed.map(c => {
      ...
      const cardHtml = `<button type="button" class="supply-card" tabindex="-1"><img src="${IMG+cards[c.id].img}" alt="${cards[c.id].name}"></button>`;
